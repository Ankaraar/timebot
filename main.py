...
TOKEN_API = '7124120933:AAE6XWlx3qBWy1P-ZPLAdKEnv3ZHYysQ6vc'
...
button4 = InlineKeyboardButton('Отправь текущее время',callback_data= 'send_datetime')
keyboard2.add(button3,button4)

@dp.message_handler(commands= 'start')
async def start(message: types.Message):
  await message.reply('Клавиатуре 1,нажми на кнопку,чтобы перейти на клавиатуру 2',reply_markup= keyboard1)
...
async def go_to_2(callback_query: types.CallbackQuery):
  await callback_query.message.edit_text('Ты перешел на 2 клавиатуру,нажми на кнопку,чтобы вернутся на 1',reply_markup= keyboard2)

@dp.callback_query_handler(lambda c: c.data == 'go_to_1')
async def go_to_2(callback_query: types.CallbackQuery):
  await callback_query.message.edit_text('Ты перешел на 1 клавиатуру,нажми на кнопку,чтобы вернутся на 2',reply_markup= keyboard1)
